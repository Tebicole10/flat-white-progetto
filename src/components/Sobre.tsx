...
const ICONOS = ['local_cafe', 'bakery_dining', 'map', 'emoji_events', 'bookmark'];

export const Sobre: React.FC<SobreProps> = ({ onCerrar }) => {
  ...
  return (
    <div className={styles.sobre}>
      <div className={styles.header}>
        <h1 className={styles.title}>Sobre el<br />proyecto</h1>
        <button className={styles.cerrar} onClick={onCerrar}>CERRAR ✕</button>
      </div>

      <div className={styles.iconRow}>
        {ICONOS.map(icon => (
          <div key={icon} className={styles.iconDisc}><Icon name={icon} size={28} color="var(--burro)" /></div>
        ))}
      </div>

      <div className={styles.texto}>
        <p className={styles.parrafoFuerte}>Flat White Progetto no nace como una declaración de principios, pero termina, inevitablemente, convirtiéndose en una.</p>
        ...
        <blockquote className={styles.cita}>¿Qué es una cafetería? ¿Qué es sino la culminación de la espiritualidad del ser humano?</blockquote>

        <p>El hombre va al café de su cuadra a leer un libro, a disfrutar un rato de su soledad, a gozar de los beneplácitos de una infusión, a intercambiar con seres amados sobre asuntos de variados tópicos. En definitiva, todo lo que sucede en la cafetería es bueno para el espíritu humano.</p>
      ...
      <button className={styles.descargaBtn} onClick={handleDescargar}>Descargar manifiesto</button>
    </div>
  );
};
