...
import { calcularPromedioCafe, calcularPromedioCategoria, calcularPrecioPromedio } from '../utils';
import type { Cafe } from '../types';
import styles from './Rankings.module.css';

type CategoriaKey = 'total' | 'cafe' | 'comestibles' | 'vajilla' | 'ambientacion' | 'servicio' | 'precio';
...
export const Rankings: React.FC = () => {
  ...
  const ranking = [...cafes]
    .filter(c => {
      ...
      if (esPrecio) return calcularPrecioPromedio(c) !== null;
      return true;
    })
    .sort((a, b) => {
  ...
  // Cafés sin precio al final (solo en modo precio)
            ...
            return (
              <div key={cafe.id}
                ...
                <div className={styles.posicion}>
                  {esMedalla
                    ? <span className={styles.medallaEmoji}>{MEDALLAS[idx]}</span>
