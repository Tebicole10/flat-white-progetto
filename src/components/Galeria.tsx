...
import { ChevronDown, Trash2, RefreshCw, Pencil } from 'lucide-react';
...
export const Galeria: React.FC<GaleriaProps> = ({ onRevisitar, onEditarCafe, onEditarVisita }) => {
  ...
  const obtenerFotoPortada = (cafe: Cafe): string | null => {
    ...
    for (const visita of visitasOrdenadas) {
      if (visita.fotos.length > 0) return visita.fotos[0];
    }
    return null;
  };
          ...
          {filtered.map(cafe => {
            ...
            return (
              <div key={cafe.id} className={`${styles.card} ${isOpen ? styles.cardOpen : ''}`}>
                <div className={styles.cardHeader} onClick={() => setExpandedId(isOpen ? null : cafe.id)}>
                  <div className={styles.cardLeft}>
                    {fotoPortada ? (
                      <img src={fotoPortada} alt={cafe.nombre} className={styles.thumb} />
                    ) : (
                ...
                {isOpen && visitaActiva && (
                  <div className={styles.detalle}>
                    ...
                    {visitaActiva.fotos.length > 0 && (
                      <div className={styles.fotos}>
                        {visitaActiva.fotos.map((f, i) => (
                          <img key={i} src={f} alt="" className={styles.foto} />
                        ))}
                      </div>
                    )}
                    ...
                    <div className={styles.ratingsGrid}>
                      {[
                        { key: 'cafe', label: '☕ Café' },
                        { key: 'comestibles', label: '🥐 Delizie' },
                        { key: 'vajilla', label: '🍽️ Vajilla' },
                        { key: 'ambientacion', label: '🎨 Ambiente' },
                        { key: 'servicio', label: '👥 Servicio' },
                      ].map(({ key, label }) => {
                        const val = visitaActiva.ratings[key as keyof typeof visitaActiva.ratings];
                        return (
